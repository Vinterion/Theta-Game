...
namespace theta {
	...
	private:
		...
		void collision_test() {
			if (_object.x_velocity != 0.f) {
				_object.object->move({ _object.x_velocity,0.f });
		...
		void falling_test() {
			if (_object.isfalling) {
				_object.object->move({ _object.x_velocity, 12.f });
	...
	public:
	  ...
	  void object_moved(const std::span<theta::Side_of_Collision> side_buffor) {
		  for(auto& it : side_buffor){
			if (it == theta::Side_of_Collision::Top && _object.y_velocity == 0.f && !_object.isfalling) {
				_object.y_velocity += 40.f;
			}
			else if (it == theta::Side_of_Collision::Right && _object.x_velocity < 24.f) {
				_object.side_of_collision = it;
				_object.x_velocity += 12.f;
				collision_test();
			}
			else if (it == theta::Side_of_Collision::Left && _object.x_velocity > -24.f) {
				_object.side_of_collision = it;
				_object.x_velocity -= 12.f;
				collision_test();
			}
